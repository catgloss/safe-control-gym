...
def plot_results(config):
    ...
    for i, n in enumerate(noise):
        ...
        path = os.path.join("./baselines/experiment_results/experiment_results", "white_noise_obs")
    ...
    name = "White Noise Observation Disturbance vs. Cost"
    np.save(os.path.join(path, config.algo + "_obs_test_no_rand"), y)
    plt.title(name)
    plt.xlabel("sigma")
    plt.ylabel("Cost")
...
def plot_comparison(config):
    ...
    plt.title(name)
    ax.legend(loc='best', frameon=False)
    plt.savefig(os.path.join(config.output_dir, "obs_noise_comparison_" + str(config.noise) + "_" + config.algo + ".jpg"))
...
if __name__ == "__main__":
    ...
    if func == "train": 
        ...
        config.task_config.init_state = { 'init_x' : 0.55, 
                                          ...
                                          'init_theta': 0.30, 
                                          'init_theta_dot': -0.0 }
    config.task_config.disturbances.observation[0] = {"disturbance_func" : "white_noise", 
                                                 "std": config.noise}
    func(config)   
