...
def plot_results(config):
    noises = [0, 10, 25, 50, 60, 70, 80, 90, 100, 110]
    ...
    np.save(os.path.join(config.eval_output_dir, config.algo + "_action_test_mse"), y_mse)
...
if __name__ == "__main__":
    ...
    if config.func == "train": 
        ...
        config.task_config.init_state = { 'init_x' : 0.55, 
                                          ...
                                          'init_theta': 0.30, 
                                          'init_theta_dot': -0.0 }
    config.task_config.disturbances.action[0] = {"disturbance_func" : "impulse", 
