...
def test_policy(config):
    ...
    if config.algo == "ppo" or config.algo == "mpsc" or config.algo == "rarl" or config.algo == "sac" or config.algo == "safe_explorer_ppo":
        ...
        if config.restore:
            control_agent.load(os.path.join(config.restore, "model_latest.pt"))
        # Test controller.
        results = control_agent.run(n_episodes=50,
                                    render=True,
...
def plot_results(config):
    noise = [0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2, 0.225, 0.25]
    fig, ax = plt.subplots()
    y = np.zeros((len(noise),1))
    x = np.asarray(noise)
    name = "plot_" + config.algo + "white_noise"
    for i,n in enumerate(noise):
        # config.output_dir = config.output_dir + "/white_noise_" + n 
