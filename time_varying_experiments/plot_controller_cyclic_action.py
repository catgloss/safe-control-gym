...
def plot_results(config):
    ...
    np.save(os.path.join(config.eval_output_dir, config.algo + "_action_" + str(config.noise) + "_test_mse"), y_mse)
    np.save(os.path.join(config.eval_output_dir, config.algo + "_action_" + str(config.noise) + "_test_ep_lengths"), y_ep_lengths)
    np.save(os.path.join(config.eval_output_dir, config.algo + "_action_" + str(config.noise) + "_test_ep_returns"), y_ep_returns)
    plt.title(name)
...
def plot_comparison(config):
    noises = [0.0, 0.5, 0.75, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
    fig, ax = plt.subplots()
    print(config.task_config.init_state)
    breakpoint()
    y_mse, y_ep_lengths, y_ep_returns = np.zeros((len(noises),1)), np.zeros((len(noises),1)), np.zeros((len(noises),1))
    x = np.asarray(noises)
    config.restore = os.path.join(config.output_dir + "/temp", sorted(os.listdir(config.output_dir + "/temp"))[-1])
    ...
    plt.xlabel("Sigma")
    plt.ylabel("Cost")
    name = config.noise_type.capitialize() + " Action Disturbance vs. Cost (With Training)"
...
if __name__ == "__main__":
    ...
    if config.noise_type == "cyclic": 
        config.task_config.env_disturbance = {'variable': 'action', 
                                              'scheduler': 'cyclic',
                                              'active_dims': [0,1],
                                              'start' : [-0.01, 0.01], 
                                              'sign': [-1, 1], 
