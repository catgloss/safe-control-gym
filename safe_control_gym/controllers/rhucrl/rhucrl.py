"""Robust Hallucinated Upper Confidence Reinforcement Learning

References papers & code:
    * [Combining Pessimism with Optimism for Robust and Efficient Model-Based Deep Reinforcement Learning](https://arxiv.org/abs/2103.10369)
...
from safe_control_gym.controllers.rhucrl.rhucrl_utils import BPTTAgent, BPTTBuffer, compute_returns_and_advantages
...
class RHUCRL(BaseController):
    ...
    def __init__(self, 
        ...
        self.agent = BPTTAgent(self.env.observation_space, self.env.action_space, **shared_agent_args)
        self.agent.to(device)

        ## This needs to be updated
        self.hallucinator = BPTTAgent(self.hallucinated_obs_space, self.hallunicated_obs_space, **shared_agent_args)

        # fetch adversary specs from env 
        if self.training:
            self.hallucinated_obs_space = self.env.get_attr("hallucinated_observation_space")[0]
            self.hallucinated_act_space = self.env.get_attr("hallucinated_action_space")[0]
            ...
            self.hallucinated_obs_space = self.env.hallucinated_observation_space
            self.hallucinated_act_space = self.env.hallucinated_action_space
        self.adversary = BPTTAgent(self.adv_obs_space, self.adv_act_space, **shared_agent_args)
        self.adversary.to(device)

        ## This needs to be updated
        self.hallucinator = BPTTAgent(self.hallucinated_obs_space, self.hallunicated_obs_space, **shared_agent_args)
        self.hallucinator.to(device)
    ...
    def train_step(self): # change this to select via the dynamical model 
    ...
    def update_agent(self): # select the optimistic agent
    ...
    def update_adversary(self): # select pessimistic agent 
    ...
    def update_hallunicator(self):
        """Updates the hallunicated dynamical model to select the pessimistic and optimistic policies."""